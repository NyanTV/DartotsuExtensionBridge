...
class KvStore {
  ...
  static T? get<T>(String key) {
    ...
    if (decoded is List) {
      ...
      if (T.toString() == 'List<int>') return List<int>.from(decoded) as T;
      if (T.toString() == 'List<double>')
        return List<double>.from(decoded) as T;
      if (T.toString() == 'List<bool>') return List<bool>.from(decoded) as T;
    }
    throw StateError(
  ...
  static Map<String, dynamic> _wrap(dynamic value) {
    ...
    if (value is List) return {'t': 'list', 'v': value.map(_wrap).toList()};
